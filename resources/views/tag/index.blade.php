            ...
            <div class="modal-content">
                ...
                <form method=POST action="{{ route('tag.store') }}">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="nome_tag" class="col-form-label">Nome:</label>
                            <input type="text" class="form-control @error('nome_tag') is-invalid @enderror" id="nome_tag" name="nome_tag">
                            @error('nome_tag')
    ...
    <table class="table table-striped table-hover bg-light mt-3">
        ...
        <tbody>
            @foreach ($tags as $tag)
                <tr>
                    <td>{{$tag->nome_tag}}</td>
                    <td>{{$tag->ativo == 1 ? 'Ativa' : 'Inativa'}}</td>
                    <td>
                    ...
                    <td>
                        <form method="POST" action="{{ route('tag.destroy', $tag) }}">
                            @csrf
                            @method('delete')
                            <a :href="route('tag.destroy', $tag)" onclick="event.preventDefault(); this.closest('form').submit();">
